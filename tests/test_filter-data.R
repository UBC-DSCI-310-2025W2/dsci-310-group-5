...
one_row_zero_output <- data.frame(a = double(),
                             b = double(),
                             c = double())
row.names(one_row_output) <- as.character()

two_row_one_zero <- data.frame(a = c(0, 1),
                      b = c(1, 2),
                      c = c(2, "3"))
two_row_one_zero_output <- data.frame(a = c(1),
                             b = c(2),
                             c = c(3))
row.names(two_row_output) <- c(2)
...
no_numeric_col <- data.frame("a" = c("0", "1", "2", "3"), 
                         "b" = c("1", "2", "3", "4"))
...
# error/abnormal case tests
test_that("`filter_data` returns a data frame with all rows containing a numeric 0 filtered out", {
  expect_error(filter_data(no_numeric_col))
})
