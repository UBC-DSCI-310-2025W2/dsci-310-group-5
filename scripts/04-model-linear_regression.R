...
Options:
  ...
  --output=<path>  Path prefix for outputs (e.g. results/linear_regression)
" -> doc
...
write.csv(train_mean, paste0(out, "/linear_regression_train_mean.csv"), row.names = FALSE)
write.csv(test_mean, paste0(out, "/linear_regression_test_mean.csv"), row.names = FALSE)
...
write.csv(preds, paste0(out, "/linear_regression_preds.csv"), row.names = FALSE)

preds_plot <- preds |>
  ggplot(aes(x = log_budget, y = log_domgross)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
ggsave(paste0(out, "/linear_regression_pred.png"), preds_plot, width = 6, height = 4, dpi = 150)

metrics <- metrics(preds, truth = log_domgross, estimate = log_domgross_preds) |>
  filter(.metric == "rmse")
write.csv(metrics, paste0(out, "/linear_regression_metrics.csv"), row.names = FALSE)
