...
#| tbl-cap: "First 6 rows of the raw data (columns 1-8)"
movie_data_raw <- read.csv("../data/raw/raw_bechdel.csv")
knitr::kable(head(movie_data_raw[, 1:8]))
```

```{r}
#| vscode: {languageId: r}
#| label: tbl-movie-data-raw-2
#| tbl-cap: "First 6 rows of the raw data (columns 9-13)"
knitr::kable(head(movie_data_raw[, 9:ncol(movie_data_raw)]))
```
...
zero_before_filter_data <- read.csv('../data/raw/raw_bechdel.csv')
```
...
In checking, we found that before filtering, there was `r sum(zero_before_filter_data$budget == 0)` movies with no budget and `r sum(zero_before_filter_data$domgross == 0)`` movie with no revenue. 
...
![Boxplots of Movie Budget and Revenue](../results/figure1-eda_boxplot.png){#fig-eda-boxplots  width=65% height=400px}
...
![Histograms of Movie Budget and Revenue](../results/figure2-eda_histogram.png){#fig-eda-histograms  width=65% height=400px}
