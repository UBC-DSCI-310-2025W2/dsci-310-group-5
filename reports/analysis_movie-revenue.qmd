...
knn_summary <- read.csv("../results/table11_knn_rmse.csv")
```
...
```{r}
#| vscode: {languageId: r}
eda_summary <- readLines("../results/eda_summary.txt")
cat(eda_summary, sep = "\n")
```
...
```{r}
#| vscode: {languageId: r}
eda_str <- readLines("../results/eda-movie_data.txt")
cat(eda_str, sep = "\n")
```
...
knn_cv_metrics <- read.csv("../results/table10_knn_cv_rmse.csv")
...
In @tbl-knn-cv-metrics, we find the cross-validated RMSE for the model at K = `r read.csv("../results/table8_knn_best_k.csv")[1]`.
