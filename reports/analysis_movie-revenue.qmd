...
Understanding the relationship between production budget and revenue can provide insight into whether increased financial investments reliably leads to higher financial returns. In this analysis, we use FiveThirtyEight's Bechdel Test dataset, which contains descriptive information, production budgets and domestic box office revenue of 1,615 films released between 1990 and 2013 to answer the question: **How does movie budget predict movie revenue?** [@fivethirtyeight2014bechdel].
...
data <- bechdel[, c("budget", "domgross")]
```
...
```{r}
#| vscode: {languageId: r}
print(paste("# of null values (before filtering):", sum(is.na(data))))

data <- data %>%
  filter(!is.na(budget), !is.na(domgross))

print(paste("# of null values (after filtering):", sum(is.na(data))))
```
...
```{r}
#| vscode: {languageId: r}
print(paste("Movies with $0 budget after filtering:", sum(data$budget == 0)))

print(paste("Movies with $0 budget after filtering:", sum(data$domgross == 0)))
```
...
```{r}
#| vscode: {languageId: r}
eda_summary <- readLines("../results/eda_summary.csv")
cat(eda_summary, sep = "\n")
```
...
```{r}
#| vscode: {languageId: r}
eda_str <- readLines("../results/eda-movie_data.csv")
cat(eda_str, sep = "\n")
```
...
lm_summary <- readLines("../results/linear_regression_summary.csv")
cat(lm_summary, sep = "\n")
```
...
metrics <- read.csv("../results/table7_linear_regression_metrics.csv")
knitr::kable(metrics)
...
knn_results <- read.csv("../results/table7_knn_tune_results.csv")
...
#| vscode: {languageId: r}
#| label: tbl-knn-min
#| tbl-cap: "The minimum RMSE is at K=88."

knn_results <- read.csv("../results/table8_knn_min.csv")
knitr::kable(knn_results)
```
...
#| tbl-cap: "Cross-validated RMSE for KNN regression model at K = 88."

knn_cv_metrics <- read.csv("../results/table8_knn_cv_metrics.csv")
knitr::kable(knn_cv_metrics)

knn_estimate_exp <- knn_cv_metrics$mean
knn_std <- knn_cv_metrics$std_err
```


In @tbl-knn-cv-metrics, we find the cross-validated RMSE for the model at `r read.csv("../results/table8_knn_best_k.csv")[1]`.
For the training set, the average RMSE across the 10 folds is `r round(knn_estimate_exp, 2)` log units with a standard error of `r round(knn_std, 2)`.
...
knn_summary <- read.csv("../results/table9_knn_metrics.csv")
...
To evaluate whether a more flexible model could improve prediction accuracy, we also trained a KNN model with cross-validation to tune the number of neighbours. The optimal value with K = `r read.csv("../results/table8_knn_best_k.csv")[1]`, producing a test RMSPE of approximately `r round(knn_estimate_exp, 2)`, very similar to the linear regression model. Since KNN did not significantly improve predictive performance, this suggests that the relationship between log-budget and log-revenue is reasonably captured by the simpler linear model. Consistent with previous research on film economics, our results suggest that while budget is associated with higher revenue, it alone cannot fully explain the variability in box office outcomes [@prag1994empirical].
